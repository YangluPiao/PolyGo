...
let translate (globals, functiondecl) =
  ...
  let primary_builder p = match p with 
    ...
    let lookup name = try StringMap.find name basic_map
  ...
  List.fold_left (init_extra) StringMap.empty whole_local  in
  ...
  let get_expr_type expr = L.type_of (List.hd expr) in
     ...
     let asn_array s e index= 
                ...
                let s' = List.map lookup arr_ID in
    ...
    let rec expr builder = function
      ...
      (match ex with A.Id s -> (match List.length (expr builder e )with 1-> let e' = List.hd (expr builder e) 
                   ...
                   | A.Arrextr (s,index) -> let name = s^(string_of_int index) in let e' = List.hd (expr builder e) 
                                            in ignore(L.build_store e' (lookup name) builder);[e']
      )
    ...
    | A.Extr e -> (match e with A.Id s -> (try  [L.build_load (lookup s) s builder;L.build_load (StringMap.find (s^"i") whole_map) (s^"i") builder]  with
                                                                        Not_found -> [L.build_load (lookup s) "tmp" builder]
                                          )
                             ...
                             | A.Arrextr (s,index)-> let name = s^(string_of_int index) in
                                                  [L.build_load (lookup name) name builder]
    ...
    | A.Mod e -> (match e with A.Complexlit (e1,e2) ->
                    ...
                    let out2 = (match image with None -> raise(Failure("Invalid complex value2."))
                                | Some v2 -> v2) in
                                expr builder (A.Primary (A.Floatlit (sqrt(out1+.out2))))
                           | A.Extr e -> (match e with A.Id s -> 
                                let e1 = L.build_load (lookup s) "tmp" builder in
